...
export function TruckCardItem({
    ...
    const localizedFuelType = fuelTypes?.[item.fuelType as keyof typeof fuelTypes] ;
    const title = getTitle(item.model, item.modelYear);
    const plate = String(item.plateNo ?? "").trim() || "-";
    const fuel = String(localizedFuelType ?? "").trim() || "-";
            ...
            >
                <Card.Body className="p-4">
                    ...
                    <View className="flex-row items-start justify-between border-b border-slate-100 pb-2.5 gap-x-3">
                        <View className="flex-1 min-w-0">
                            ...
                            <Text
                                ...
                                {labels.fuelType}{" "}
                                <Text
                                    className={`font-semibold text-[#3F5F87] ${getMyanmarLeadingClass(locale)}`}>{fuel}</Text>
                            </Text>
                        </View>
                    ...
                    <View className="mt-3 gap-y-3">
                        ...
                        <View
                            className="bg-[#EAF1F8]/30 border border-[#EAF1F8] rounded-lg p-2.5 flex-row justify-between gap-x-4">
                            <View className="flex-1 min-w-0">
                                <Text
